...
bool ipv6_map_ip_to_ll(struct protocol_interface_info_entry *cur, struct ipv6_neighbour *n, const uint8_t ip_addr[16], addrtype_e *ll_type, const uint8_t **ll_addr_out);
bool ipv6_map_ll_to_ip_link_local(struct protocol_interface_info_entry *cur, addrtype_e ll_type, const uint8_t *ll_addr, uint8_t ip_addr_out[16]);
typedef uint16_t (ll_addr_handler_t)(int8_t nwk_id, addrtype_e addr_type, const uint8_t *addr_ptr);
