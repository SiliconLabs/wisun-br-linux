...
extern "C" {
#include "tools/silabs-ws-dc/dc.h"
...
static void dc_ns3_cleanup(void *arg)
{
...
void dc_ns3_main(const char *config)
{
    char config_filename[] = "/tmp/silabs-ws-dc-ns3-XXXXXX.conf";
    ...
    // Cast to non-const, dc is trusted to not modify its arguments
    args.push_back((char *)"silabs-ws-dc");
    ...
    pthread_cleanup_push(dc_ns3_cleanup, config_filename);
    dc_main(args.size() - 1, (char **)args.data()); // Does not return
...
extern "C" void __wrap_exit(int status)
{
    if (strlen(last_error))
        fprintf(stderr, "\x1b[31mdc: %s\x1b[0m\n", last_error);
