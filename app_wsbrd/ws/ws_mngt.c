                        ...
                        const struct mcps_data_rx_ie_list *ie_ext)
{
    ...
    if (!ws_neigh)
        return;
    ws_neigh_ut_update(&ws_neigh->fhss, ie_utt.ufsi, data->hif.timestamp_us,
                         ...
                         const struct mcps_data_rx_ie_list *ie_ext)
{
    ...
    ws_neigh_ut_update(&ws_neigh->fhss, ie_utt.ufsi,
                       data->hif.timestamp_us, &EUI64_FROM_BUF(data->SrcAddr));
    ws_neigh_us_update(&ws_info->fhss_config, &ws_neigh->fhss, &ie_us.chan_plan,
                             ...
                             const struct ws_neigh *dst)
{
    struct ws_llc_mngt_req req = {
        ...
        .wh_ies.flus    = true,
        .wh_ies.lbs     = true,
        .wh_ies.lto     = dst->lto_info.needs_lto,
        .wh_ies.panid   = true,
        .wp_ies.bs      = true,
                             ...
                             const struct ws_neigh *dst)
{
    struct ws_llc_mngt_req req = {
        ...
        .wh_ies.utt      = true,
        .wh_ies.lbt      = true,
        .wh_ies.lto      = dst && dst->lto_info.needs_lto,
