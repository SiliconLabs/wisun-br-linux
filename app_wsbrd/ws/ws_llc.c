                           ...
                           const struct mcps_data_rx_ie_list *conf_data)
{
    ...
    if (msg->security.SecurityLevel == IEEE802154_SEC_LEVEL_ENC_MIC64 &&
        data_cpy.hif.status == HIF_STATUS_SUCCESS)
        auth_update_frame_counter(net_if->auth, msg->security.KeyIndex,
                                  data_cpy.hif.frame_counter);
    ...
    if (ws_neigh) {
        if (data_cpy.sec.SecurityLevel == IEEE802154_SEC_LEVEL_ENC_MIC64) {
            FATAL_ON(data->sec.KeyIndex < 1 || data->sec.KeyIndex > 7, 3);
                                ...
                                const struct mcps_data_rx_ie_list *ie_ext)
{
    ...
    if (!ws_neigh) {
        ...
        ws_neigh_del(&net_if->ws_info.neighbor_storage, &ws_neigh->eui64);
        add_neighbor = true;
    }
    if (add_neighbor) {
                              ...
                              const struct mcps_data_rx_ie_list *ie_ext)
{
    ...
    if (neigh && data->Key.SecurityLevel) {
        ...
        if (neigh->frame_counter_min[data->Key.KeyIndex - 1] > data->Key.frame_counter ||
            ...
            TRACE(TR_DROP, "drop %-9s: frame cnt=%u < cnt-min=%u for key-idx=%u src=%s",
...
static void ws_llc_rate_handle_tx_conf(llc_data_base_t *base, const mcps_data_cnf_t *data, struct ws_neigh *neighbor)
{
    ...
    if (neighbor->ms_tx_count + neighbor->ms_retries_count > MS_FALLBACK_MIN_SAMPLE) {
        if (neighbor->ms_retries_count > 4 * neighbor->ms_tx_count) {
            ...
            WARN("mode switch disabled for %s with phy_mode_id %d",
                 tr_eui64(neighbor->eui64.u8), neighbor->ms_phy_mode_id);
