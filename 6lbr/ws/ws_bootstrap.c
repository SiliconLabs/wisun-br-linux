...
void ws_bootstrap_fhss_configure_channel_masks(struct net_if *cur, fhss_ws_configuration_t *fhss_configuration)
{
    ...
    ws_common_generate_channel_list(&cur->ws_info.fhss_conf, fhss_configuration->domain_channel_mask, cur->ws_info.fhss_conf.number_of_channels, cur->ws_info.hopping_schedule.regulatory_domain, cur->ws_info.fhss_conf.operating_class, cur->ws_info.fhss_conf.channel_plan_id);
}
