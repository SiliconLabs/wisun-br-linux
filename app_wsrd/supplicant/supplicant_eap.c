...
{
    bool must_fragment = pktbuf_len(&supp->tls_client.io_tx) > WS_MTU_BYTES;
    uint32_t tx_len = pktbuf_len(&supp->tls_client.io_tx);
    struct pktbuf buf = { };
    uint8_t flags = 0;
    ...
    pktbuf_push_tail(&buf, NULL, MIN(pktbuf_len(&supp->tls_client.io_tx), WS_MTU_BYTES));
    pktbuf_pop_head(&supp->tls_client.io_tx, pktbuf_head(&buf),
                    MIN(pktbuf_len(&supp->tls_client.io_tx), WS_MTU_BYTES));
...
static void supp_eap_do_handshake(struct supp_ctx *supp, const struct eap_hdr *eap_hdr)
{
    ...
    if (pktbuf_len(&supp->tls_client.io_tx) || ret != MBEDTLS_ERR_SSL_WANT_READ)
...
static void supp_eap_tls_recv(struct supp_ctx *supp, const struct eap_hdr *eap_hdr, struct iobuf_read *iobuf)
{
    ...
    if (supp->expected_rx_len && !FIELD_GET(EAP_TLS_FLAGS_MORE_FRAGMENTS_MASK, flags) &&
        pktbuf_len(&supp->tls_client.io_rx) + remaining_size != supp->expected_rx_len) {
    ...
    pktbuf_push_tail(&supp->tls_client.io_rx, iobuf_pop_data_ptr(iobuf, remaining_size), remaining_size);
