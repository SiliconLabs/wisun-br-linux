                             ...
                             const void *buf, size_t buf_len, struct ipv6_neigh *nce)
{
     ...
     * Upon receipt of a Registration Refresh Request, child nodes [...] MUST
     * ignore the ROVR field (EUI-64)
     */
    if (!eui64_eq(&ipv6->eui64, &eui64) && aro->status != NDP_ARO_STATUS_REFRESH) {
        TRACE(TR_DROP, "drop %-9s: eui64 mismatch", "na(aro)");
        return;
    }
