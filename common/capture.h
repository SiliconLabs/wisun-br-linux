 ...
 * socket reads.
 *
 * Networking file descriptors such as TUN device and sockets must be
 * registered using capture_register_netfd(). Regular HIF frames received by
 * the RCP must be recorded by calling capture_record_hif(). File descriptor
 * interactions must use the xread()/xwrite() variants, which call the regular
 * read()/write() and write data to the capture file only after capture_start()
 * is called to set the output file. Finally, all random number generation must
 * use xgetrandom() to ensure reproducibility.
 */
