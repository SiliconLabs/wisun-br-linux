...
void auth_eap_send_request_identity(struct auth_ctx *auth, struct auth_supp_ctx *supp)
{
    ...
    if (auth->radius_fd < 0) {
        if (!supp->eap_tls.tls) {
            supp->eap_tls.tls = zalloc(sizeof(struct tls_client_ctx));
...
void auth_eap_cleanup(struct auth_supp_ctx *supp)
{
    ...
    free(supp->eap_tls.tls);
    supp->eap_tls.tls = NULL;
}
...
static void auth_eap_recv_resp_tls(struct auth_ctx *auth, struct auth_supp_ctx *supp, struct iobuf_read *iobuf)
{
    ...
    if (!supp->eap_tls.tls) {
        TRACE(TR_DROP, "drop %-9s: no ongoing handshake", "eap-tls");
        return;
    }
    if (iobuf->err) {
    ...
    if (FIELD_GET(EAP_TLS_FLAGS_LENGTH_MASK, flags)) {
        ...
        if (supp->eap_tls.frag_expected_len && expected_rx_len != supp->eap_tls.frag_expected_len) {
            TRACE(TR_DROP, "drop %-9s: length mismatch with initial fragment", "eap-tls");
            return;
        }
        supp->eap_tls.frag_expected_len = expected_rx_len;
    }
